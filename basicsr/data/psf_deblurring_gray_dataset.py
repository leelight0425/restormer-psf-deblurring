...
applies geometric augmentation before randomly cropping patches, and applies
spatially-varying PSF convolution to create blurry LQ images.
...
class Dataset_PSFDeblurringGray(data.Dataset):
    ...
    def _psf_convolve_gray(image, psf):
        ...
        """
        img_2d = np.ascontiguousarray(np.squeeze(image), dtype=np.float64)
        psf_2d = np.ascontiguousarray(np.squeeze(psf), dtype=np.float64)
        convolved = convolve(img_2d, psf_2d, mode='reflect')
    ...
    def __getitem__(self, index):
        ...
        # ---- 5. PSF convolution: augmented GT -> LQ (grayscale domain) ----
        ...
        # ---- 8. Normalize (optional, single-channel) ----
